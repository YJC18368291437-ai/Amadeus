...
export function createConverter({ executable = 'docbuilder', mode = 'native', image = DEFAULT_IMAGE, dockerExecutable, fontsDir, cacheVersion = '1', cacheDir = path.join(os.tmpdir(), 'cofolio-preview-cache'), timeoutMs = 120000, maxFileBytes = 100 * 1024 ** 2, maxCacheBytes = 512 * 1024 ** 2, workers = 1, office = createOnlyOffice({ executable, mode, image, dockerExecutable, fontsDir, timeoutMs, workers }) } = {}) {
