...
function Files({ sessionId, useTabInfo }) {
  ...
  async function uploadFiles(selected) {
    ...
    try {
      for (let i = 0; i < selected.length; i++) {
        ...
        while (true) {
          try { await request(fileUrl('upload', sessionId, target, extra), { method: 'PUT', body: file, signal: controller.current.signal }); break; }
            ...
            if (choice === 'cancel') { controller.current.abort(); throw new Error('上传已取消'); }
      ...
      await refresh();
    } catch (error) { setError(error.name === 'AbortError' ? '上传已取消，已完成的文件会保留。' : error.message); await refresh(); }
  ...
  async function chooseFolder() {
    ...
    if (!window.showDirectoryPicker) { setError('当前浏览器仅支持按文件上传目录，空文件夹无法保留。使用支持目录选择的 HTTPS 浏览器可完整上传。'); folder.current.click(); return; }
    try {
      ...
      async function walk(directory, prefix) {
        ...
        for await (const [name, entry] of directory.entries()) {
          if (entries.length >= 100000) throw new Error('单次上传最多支持 100000 个目录项');
